    ...
    UUPSUpgradeable
{
    ...
    modifier onlyListed(uint _tokenId) {
        require(
            ...
            "This NFT doesn't listed"
        );
        _;
    }
    modifier onlyNftAuthor(uint _tokenId) {
        require(
            listDetail[_tokenId].author == _msgSender(),
    ...
    function updatePrice(
        uint256 _tokenId,
        uint _price
    ) external onlyListed(_tokenId) onlyNftAuthor(_tokenId) {
    ...
    function buyNft(
        uint256 _tokenId,
        uint256 _price
    ) external onlyListed(_tokenId) {
        address sender = _msgSender();
        require(
            token.balanceOf(sender) >= _price,
